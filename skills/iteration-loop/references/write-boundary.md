...
The snapshot itself lives **outside the project directory entirely** (`verify_gate_snapshot_path`, a deterministic path under `$TMPDIR`), not as a sibling file next to `backlog.json`. An earlier version kept it as a sibling file; a worker with the same Edit/Write/Bash access used to tamper with `backlog.json` could just as easily overwrite a sibling snapshot to match, laundering the comparison instead of failing it. Living outside the tree relies on Claude Code's own project-directory scoping of file tools as the actual boundary — the path isn't secret, being unreachable is what protects it, not obscurity.
...
`scripts/lib/backlog.sh` is the only code path with permission to mutate a step's `status` field in `backlog.json` (item status is never stored — it's derived from the steps), and it does so only in response to a verify-gate result — never in response to anything the worker said about its own progress. If code-virtuoso's `acceptance-verifier` agent (or an equivalent) is available and configured to run, its verdict is logged to `progress.log` as an advisory note alongside the mechanical result, but it never overrides the mechanical gate.
